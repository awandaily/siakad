 ...
 <div class="row clearfix" id="area_lod">
 		...
 		<div class="card">
 			<div class="header row">
 				<div class="col-md-3 " style="padding-bottom:15px">
 					<h2 style='font-size:16px'><b>Catatan Siswa</b></h2>
 				</div>
 	...
 	<script>
 		...
 		function hapus_tanggapan(id) {
 			alertify.confirm("<center>Hapus tanggapan ini ?</center>", function() {
 				$.post("<?php echo site_url("bpbk/hapus_bpbk"); ?>", {
 					id: id
 				}, function() {
 					reload_table();
 				})
 	...
 	<script type="text/javascript">
 		...
 		var dataTable = $('#table').DataTable({
 			...
 			"ajax": {
 				"url": "<?php echo site_url('bpbk/getCatatan'); ?>",
 				"type": "POST",
 				"data": function(data) {
 	...
 	<script>
 		function log_tanggapi(id) {
 			...
 			$.post("<?php echo base_url(); ?>catatan/viewTanggapi", {
 				id: id
 			}, function(data) {
 					...
 					<div class="modal-body">
 						<div class="row clearfix">
 							...
 							<?php
								...
								$cks = $this->db->get("data_pegawai")->num_rows();
								...
								?>
 								<div class="col-lg-12 col-md-12 col-xs-12 ">
 									<div class="form-groups">
 										...
 										<div class="form-line">
 											<textarea class="form-control" required name="f[tanggapan_ks]"></textarea>
 										</div>
 									</div>
 									<div class="form-group">
 										<label>Status Pengaduan</label><br>
 										<div class="form-line">
 											<select class="form-control" name="f[status_ks]">
 												<option value="">- Pilih -</option>
 												<option value="Close">Close</option>
 											</select>
 										</div>
 										<input type="hidden" name="f[id_ks]" value="<?php echo $this->session->userdata("id"); ?>" \>
 									</div>
 								</div>
 							...
 							<?php } else { ?>
 								...
 								<?php
									//$idj = $this->m_reff->goField("data_pegawai", "id_jabatan", "WHERE id = '".$this->session->userdata("id")."' ");
									//$nmj = $this->m_reff->goField("tr_jabatan", "nama", "WHERE id='".$idj."' ");
									...
									?>
 									...
 									<div class="col-lg-12 col-md-12 col-xs-12 ">
 										<div class="form-groups">
 											<label>Tanggapan Anda </label>
 											<div class="form-line">
 												<textarea class="form-control" required name="f[tanggapan_bp]"></textarea>
 											</div>
 										</div>
 										<div class="form-group">
 											<label>Status Pengaduan</label><br>
 											<div class="form-line">
 												<select class="form-control" name="f[status_bp]">
 													<option value="">- Pilih -</option>
 													<option value="Open">Open</option>
 													<option value="Close">Close</option>
 												</select>
 											</div>
 											<input type="hidden" name="f[id_bp]" value="<?php echo $this->session->userdata("id"); ?>" \>
 										</div>
 									</div>
 								...
 								<?php } else { ?>
 									...
 									<div class="col-lg-12 col-md-12 col-xs-12 ">
 										<div class="form-groups">
 											<label>Tanggapan Anda </label>
 											<div class="form-line">
 												<textarea class="form-control" required name="f[tanggapan_kes]"></textarea>
 											</div>
 										</div>
 										<div class="form-group">
 											<label>Status Pengaduan</label><br>
 											<div class="form-line">
 												<select class="form-control" name="f[status_kes]">
 													<option value="">- Pilih -</option>
 													<option value="Open">Open</option>
 													<option value="Close">Close</option>
 												</select>
 											</div>
 											<input type="hidden" name="f[id_kes]" value="<?php echo $this->session->userdata("id"); ?>" \>
 										</div>
 									</div>
