...
foreach($data as $dataDB){ 
		...
		$masuk=0;$alfa=0; $kolom=0;$hasilDB="";$efektif=0;$jamMasuk=0;$jamTelat=0;
   		  	  	for($i=0;$i<$jml;$i++){
					$tgl=$this->tanggal->tambah_tgl($tgl1o,$i);
			 $hasil=$this->mdl->cekAbsenFinger($dataDB->nip,$tgl);	$nip=$dataDB->nip;
			 if($hasil==1){//ada
			 ...
			 $pulang=$this->mdl->absenPulang($tgl,$nip);
			  ...
			  if($sts!="0"){
			     ...
			     $hasilDB='<td style="background-color:#7CFC00"> <font color="#7CFC00">`</font>'.$datang.'</td>
			             <td style="background-color:#7CFC00"><font color="#7CFC00">`</font>'.$pulang.' </td>';
			   ...
			   //  $hasil='<td  style="background-color:#F08080">&#10006;</td>';
			      $hasilDB='<td style="background-color:#F08080"> <font color="#F08080">`</font>'.$this->mdl->absenDatang($tgl,$nip).' </td>
			             <td style="background-color:#F08080"><font color="#F08080">`</font>'.$this->mdl->absenPulang($tgl,$nip).' </td>';
									...
									<th  style='background-color:#009688;color:white;font-weight:bold' rowspan="3"  >STATUS</th>  
									 ...
									 <th  style='background-color:#009688;color:white;font-weight:bold'   colspan="<?php echo $kolom;?>"  align="center" >PERIODE <?php echo $tgl1. " - ". $tgl2?></th>
										...
										<th  style='background-color:#009688;color:white;font-weight:bold' rowspan="3"  >TOTAL JAM MASUK  </th> 
											<th  style='background-color:#009688;color:white;font-weight:bold' rowspan="3"  >TOTAL JAM TELAT  </th> 
									 </tr>
