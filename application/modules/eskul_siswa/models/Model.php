...
class Model extends CI_Model  {
	...
	private function _get_datatables_open($tbl){
		$filter="";
			...
			//$filter.="AND ".$tbl.".id_eskul='".$this->ids()."'";
	
		if ($this->session->userdata("level") == "ORTU") {
			$idu = $this->m_reff->goField("data_siswa", "id", "WHERE ai='".$this->session->userdata("id")."'");
		}
		else{
	...
	$sl = "
		".$tbl.".*,
		tr_ektrakurikuler.id AS eskul_id,
		tr_ektrakurikuler.nama AS eskul,
		tr_ektrakurikuler.kode AS eskul_kode,
		data_pegawai.id AS pegawai_id,
		data_pegawai.nip,
		data_pegawai.nama AS pembina
	";


	$query="SELECT ".$sl." FROM ".$tbl.", tr_ektrakurikuler, data_pegawai  WHERE ".$tbl.".id_eskul = tr_ektrakurikuler.id AND tr_ektrakurikuler.kode = data_pegawai.nip AND ".$tbl.".j_siswa LIKE '%\"".$idu."\"%'  $filter ";

		if($_POST['search']['value']){
			$searchkey=$_POST['search']['value'];
			$query.=" AND (
			 tr_ektrakurikuler.nama LIKE '%".$searchkey."%'  
			) ";
		}

		$column = array('', 'tr_ektrakurikuler.nama'  );
		$i=0;
		...
		{
		$query.=" order by tr_ektrakurikuler.nama asc";
		} 
		else if(isset($order))
		{
			$order = $order;
	...
	function ids()
	{
		...
		$dp=$this->m_reff->dataProfileSiswa(); 
		return $this->m_reff->goField("eskul_anggota","id","where j_siswa LIKE '%\"".$dp->id."\"%' ");
	}
