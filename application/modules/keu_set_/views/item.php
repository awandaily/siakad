                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="header">
                            <h2>DATA REFERENSI ITEM PENGELUARAN</h2>
  ...
  <script type="text/javascript">
  	  function hapus(id,akun){
		   ...
		   $.post("<?php echo site_url("keu_set/hapus/keu_tr_pengeluaran"); ?>",{id:id},function(){
			   reload_table();
		      })
		   })
	  };
   ...
   // $(document).ready(function() {
      table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('keu_set/data/keu_tr_pengeluaran');?>",
            "type": "POST",
...
function add()
{
	...
	$("form").attr('url',"<?php echo base_url()?>keu_set/insert/keu_tr_pengeluaran");
	...
	<form  action="javascript:saveModal('modal_artikel')" id="f_modal_artikel" url="<?php echo base_url()?>profile/insert"   method="post" enctype="multipart/form-data">
                    <div class="modal-content"> <span  title="tutup"  data-dismiss="modal" class="    pull-right waves-effect"><i class="material-icons">cancel</i> </span>
                        <div class="modal-header">
                            <h4 class="modal-title col-indigo" id="defaultModalLabel">Data Kategori Pengeluaran </h4>
                        ...
                        <div class="modal-body">
                         ...
                         <input type="hidden" readonly name="id_" >
								...
								<label class="form-label" style="margin-top:-5px">Kategori Pengeluaran</label>
                                    <div class="form-line ">
                                       <?php
									   $db=$this->db->get("keu_tr_kategori_pengeluaran")->result();
									   foreach($db as $db)
									   {
									   $opsi[$db->kode]=$db->kode;
									   }
									   echo form_dropdown("f[kode_kategori]",$opsi,"","class='form-control'");
									   ?>
								...
								<label class="form-label" style="margin-top:-5px">Kode</label>
                                    <div class="form-line ">
                                        <input type="text" class="form-control" name="f[kode]"   required>
									 	...
									 	<div class="form-group  ">  <label class="form-label" style="margin-top:-5px">Nama</label>
                                    <div class="form-line    ">
...
$("#imgInp").change(function() {
 ...
 function edit(id,nama,kode,kate)
	 {	 
		 			
			 	$("[name='id']").val(id);
			 	$("[name='id_']").val(id);
			 	$("[name='f[kode_kategori]']").val(kate);
			 	$("[name='f[kode]']").val(kode);
			 	$("[name='f[nama]']").val(nama);
				$("#modal_artikel").modal({backdrop: 'static',drugged:true, keyboard: false});
				$("form").attr('url',"<?php echo base_url()?>keu_set/update/keu_tr_pengeluaran");
	 }
