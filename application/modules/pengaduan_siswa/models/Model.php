...
class Model extends CI_Model  {
	...
	function __construct()
    {
	 ...
	 function get_open($tbl, $id_guru)
	{
		$query=$this->_get_datatables_open($tbl, $id_guru);
		if($_POST['length'] != -1)
		$query.=" limit ".$_POST['start'].",".$_POST['length'];
		return $this->db->query($query)->result();
	}
	public function count_file($tbl, $id_guru)
	{				
		$query = $this->_get_datatables_open($tbl, $id_guru);
	...
	private function _get_datatables_open($tbl, $id_guru){
		$filter="";
		  
		if ($id_guru!="") {

			if ($id_guru=="SEKOLAH") {
				$filter.= "AND id_jenis = '2' ";
			}
			else{
