...
class Model extends CI_Model  {
	...
	{
		return 	$return=$this->m_reff->goField("keu_tagihan_pokok","nama_tagihan","where id_tagihan='".$id."'");
	...
	//	$filter.=" AND tgl_bayar BETWEEN '".$tgl1."' AND '".$tgl2."' ";
		...
		$query="SELECT keu_tm_bayar.* ,SUM(keu_tm_bayar.nominal_bayar)  AS nominal, data_siswa.id_kelas, tm_kelas.id_tk FROM keu_tm_bayar, data_siswa, tm_kelas  where 1=1 AND keu_tm_bayar.id_siswa = data_siswa.id AND data_siswa.id_kelas = tm_kelas.id  ".$filter." GROUP BY tgl_bayar,id_siswa ";
			if($_POST['search']['value']){
			$searchkey=$_POST['search']['value'];
				$query.=" AND (
				nama_tagihan LIKE '%".$searchkey."%'  
	...
	{
		$query=$this->_getHistoryDetail();
		if($_POST['length'] != -1)
		$query.=" limit ".$_POST['start'].",".$_POST['length'];
		return $this->db->query($query)->result();
	}
	function _getHistoryDetail()
	{	$filter="";
		$tgl=$this->input->post("tgl");
	//	$tgl1=$this->tanggal->range_1($tgl);
    //		$tgl2=$this->tanggal->range_2($tgl);
	...
	//	$filter.=" AND tgl_bayar BETWEEN '".$tgl1."' AND '".$tgl2."' ";
		...
		$query="SELECT keu_tm_bayar.* ,keu_tm_bayar.nominal_bayar  AS nominal, data_siswa.id_kelas, tm_kelas.id_tk FROM keu_tm_bayar, data_siswa, tm_kelas  where 1=1 AND keu_tm_bayar.id_siswa = data_siswa.id AND data_siswa.id_kelas = tm_kelas.id  ".$filter;
			if($_POST['search']['value']){
			$searchkey=$_POST['search']['value'];
				$query.=" AND (
				nama_tagihan LIKE '%".$searchkey."%'  
	...
	public function count__getHistoryDetail()
	{				
		$query = $this->_getHistoryDetail();
        return  $this->db->query($query)->num_rows();
	}
	...
	{	
		...
		$tgl_sesuai=$this->input->post("tgl_sesuai");		$tgl_bayar=$this->input->post("tgl_bayar");
		...
		$db=$this->db->query("SELECT *,id_tagihan AS kode,COUNT(*) AS kelipatan FROM keu_tagihan_pokok WHERE id_siswa='".$id_siswa."' GROUP BY id_tagihan,jenis_tagihan ")->result();		
		foreach($db as $val)
		{
         ...
         $tmp    = $_FILES[$file_form]['tmp_name']; 
				...
				if($nis and $tglDb){
					...
					if(!$cekKode){
		          	...
		          	$db=$this->db->query("SELECT *,id_tagihan AS kode,COUNT(*) AS kelipatan FROM keu_tagihan_pokok WHERE id_siswa='".$id_siswa."'  AND id_tagihan='".$kode."'
                                     GROUP BY id_tagihan,jenis_tagihan")->result();	
										foreach($db as $val)
										{
											$input=$nominal;
