...
class Model extends CI_Model  {
    ...
    function cekIzinHarian($tgl)
    {
        return $this->db->query("select * from tm_guru_izin where id_guru='".$this->mdl->idu()."' and  CAST('".$tgl."' AS DATE) BETWEEN `start` AND `end` ")->row();
    }
    function cekInval($tgl,$id_jadwal)
	{   $idguru=$this->idu();
         ...
         $this->db->set("sumber",$id);
          $this->db->set("id_guru",$idguru);
           $this->db->set("id_mapel",$idmapel);
            ...
            if($id==5){
                $izin="PKL";
                $this->db->set("izin","PKL");
                $jml_jam_valid=$this->m_reff->jamValid($jam,"");	
                $this->db->set("jml_jam_valid",$jml_jam_valid);
            }
         
           
           if(($id!=3 and $id!=5) or $izin==false){
               ...
               if($cek)
               {
                   ...
                   if($id==1){
                      ...
                      $cekkbm=$this->m_reff->pengaturan(22);
                       if($cekkbm=="ya"){
                 ...
                 //  $cek=$this->db->query("select * from tm_absen_guru where id_jadwal='".$idjadwal."' and SUBSTR(tgl,1,10)='".date('Y-m-d')."' limit 1")->num_rows();
                   if(!$cek){
                     return  $this->db->insert("tm_absen_guru");
                   }else{
                       $this->db->where("id_jadwal",$idjadwal);
