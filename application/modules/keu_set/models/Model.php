...
class Model extends CI_Model  {
	...
	{
	   $n =  $this->input->post("nominal");
	   $n =  str_replace(".","",$n);
	   $t =  $this->input->post("tgl_bayar");
	   $t =  $this->tanggal->eng_($t,"-");
	   $j =  $this->input->post("jurusan");
	   
	   foreach($j as $j)
	   {
	       
	       $this->mdl->setSpp($n,$j,$t);
	...
	public function updateSpp(){
		$siswa = $_POST["siswa"];
		$awal  = $this->tanggal->eng_($_POST["awal"], "-");
		$akhir = $this->tanggal->eng_($_POST["akhir"], "-");
		$baru  = $_POST["baru"];
        $baru  = str_replace(".","",$baru);
        
		for ($i=0; $i < count($siswa); $i++) {
			$this->db->set("tagihan", $baru);
			$this->db->where("tgl_tagihan>=", $awal);	
			$this->db->where("tgl_tagihan<=", $akhir);
			$this->db->where("tgl_bayar", NULL);
			$this->db->where("id_tagihan", "spp");
			$this->db->where("id_siswa", $siswa[$i]);
	...
	function setSpp($nominal,$jurusan,$tgl)
	{       
	    $kls=$this->db->query("SELECT id FROM tm_kelas WHERE id_jurusan IN ($jurusan)")->result();
	    $kelas="";
	    foreach($kls as $k)
	    {
	        $kelas.=$k->id.",";
	    ...
	    $kelas=substr($kelas,0,-1);
	        $this->db->where("id_siswa IN (select id from data_siswa where id_kelas in($kelas) AND id_sts_data in (1,4) ) ");
	       $this->db->set("tagihan",$nominal);
	       $this->db->where("tgl_tagihan >=",$tgl);
	        $this->db->where("(bayar is null or bayar=0)"); 
	        $this->db->where("id_tagihan","spp");
