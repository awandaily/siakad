...
Class M_pemasukan extends ci_model {
    ...
    function get_datapemasukan() {
        $query = $this->_get_datapemasukan();
        if ($_POST['length'] != -1)
    ...
    private function _get_datapemasukan() {
        ...
        if ($grafik == "tdetail" or $grafik == "gdetail") {
            $group_by = "";
            $select = "nominal,tgl,nama,ket,id,tipe,nama,id_siswa";
        }
        if ($grafik == "th" or $grafik == "gh") {
            $group_by = "group by SUBSTRING(tgl,1,10)";
            $select = "SUM(nominal) as nominal,tgl,nama,ket,id,tipe,nama,id_siswa";
        }
        if ($grafik == "tm" or $grafik == "gm") {
            $group_by = "group by YEARWEEK(SUBSTRING(tgl,1,10))";
            $select = "CONCAT('minggu ke:',WEEK(tgl),'-',YEAR(tgl)) AS tgl,SUM(nominal) AS nominal,ket,id,tipe,nama,id_siswa";
        }
        if ($grafik == "tb" or $grafik == "gb") {
            $group_by = "group by MONTH(SUBSTRING(tgl,1,10))";
            $select = "CONCAT(MONTH(tgl),'-',YEAR(tgl)) AS tgl,SUM(nominal) AS nominal,ket,id,tipe,nama,id_siswa";
        }
        if ($grafik == "tt" or $grafik == "gt") {
            $group_by = "group by YEAR(SUBSTRING(tgl,1,10))";
            $select = "YEAR(tgl) AS tgl,SUM(nominal) AS nominal,ket,id,tipe,nama,id_siswa";
        }



        $query = "SELECT " . $select . " FROM v_pemasukan WHERE 1=1 $tgl $filter ";
    ...
    function totalPemasukanPeriode($awal, $akhir,$tipe) {
		if($tipe!="all")
		{
    ...
    function exportData($tanggal, $grafik,$tipe) {
      ...
      //  $akhir = $this->tanggal->rangeindo($tanggal, 1);
        ...
        $database = $this->_get_datapemasukan();
