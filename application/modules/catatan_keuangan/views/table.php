						...
						<form action="#"  method="post" id="area_lod">
							<table id='table' class="tabel black table-bordered  table-hover dataTable" style="font-size:12px;width:100%">
										<thead  class='sadow bg-blue'>
											...
											<th class='thead' width='110px'>&nbsp;<i class="icon-shopping-cart  "></i> Nama Pengeluaran</th>
											<?php } ?>
											<th class='thead' width='110px'>&nbsp;<i class="icon-shopping-cart "></i> Nominal Pengeluaran</th>
											<?php if($grafik=="tdetail"){?>
    ...
    var table;
      ...
      table = $('#table').DataTable({ 
       ...
       "processing": false, //Feature control the processing indicator.
        ...
        "ajax": {
            "url": "<?php echo base_url();?>catatan_keuangan/getListPengeluaran?grafik=<?php echo $grafik; ?>&tanggal=<?php echo $tanggal; ?>",
            "type": "POST",
			"data": function ( data ) {
			 	
						  data.tipe= $('[name="tipe[]"]').val();
						  data.kode= $('[name="fkode[]"]').val();
		 },beforeSend: function() {
               loading("area_lod");
            },
			complete: function() {
