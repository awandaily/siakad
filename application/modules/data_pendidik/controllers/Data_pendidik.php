...
class Data_pendidik extends CI_Controller
{
	...
	public function rekap_input_nilai()
	{

		$ajax = $this->input->get_post("ajax");
		if ($ajax == "yes") {
			// 	echo "<script> window.location.href='pendidik';</script>";
			echo	$this->load->view("rekap_input_nilai");
		} else {
			$data['konten'] = "rekap_input_nilai";
			$this->_template($data);
		}
	}
	...
	function get_pertemuan()
	{
		...
		$d = $this->db->get("tm_absen_guru")->result();
	...
	function data()
	{
		...
		foreach ($list as $dataDB) {
			...
			$row[] = $jml_jam_masuk;
			$jmlJamKeseluruhan = $this->m_guru->totalJamMengajar($dataDB->id);
			...
			$row[] = "<a href='javascript:void(0)' class='size' onclick='kelas(`" . $dataDB->id . "`,`" . $dataDB->nama_lengkap . "`)'><b>  " . $jmlJamKeseluruhan . "</b></a>";
	...
	function pendidik_pdf()
	{

		ob_start();
		$isi = $this->load->view('pendidik_pdf');
		// return true;
		$isi = ob_get_clean();
		...
		//$this->load->view('pendidik_pdf');
	}

	function data_input_nilai()
	{
		$list = $this->mdl_pendidik->get_data_guru();
		$data = array();
		$no = $_POST['start'];
		$no = $no + 1;
		foreach ($list as $dataDB) {
			...
			$nilai = $this->mdl_pendidik->get_nilai($dataDB->id);

			if ($nilai["uh"] == "1") {
				$uh = '<i class="material-icons col-green">check_circle</i>';
			} else {
				$uh = '<i class="material-icons col-blue-grey">highlight_off</i>';
			}

			if ($nilai["uts"] == "1") {
				$uts = '<i class="material-icons col-green">check_circle</i>';
			} else {
				$uts = '<i class="material-icons col-blue-grey">highlight_off</i>';
			}

			if ($nilai["uas"] == "1") {
				$uas = '<i class="material-icons col-green">check_circle</i>';
			} else {
				$uas = '<i class="material-icons col-blue-grey">highlight_off</i>';
			}
			...
			$row[] = "<a href='javascript:void(0)' class='size linehover'  onclick='detail(`" . $dataDB->id . "`,`" . $dataDB->nama_lengkap . "`)' >  [" . $dataDB->id . "]" . $dataDB->nama_lengkap . " </a>";
			$row[] = "<span><center>" . $uh . "</center></span>";
			$row[] = "<span><center>" . $uts . "</center></span>";
			$row[] = "<span><center>" . $uas . "</center></span>";
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $c = $this->mdl_pendidik->count_guru(),
			"recordsFiltered" => $c,
			"data" => $data,
		);
		//output to json format
	...
	function data_rpp()
	{
		...
		foreach ($list as $dataDB) {
			...
			$dbq = $this->db->query("select rpp from v_mapel_ajar where id_guru='" . $dataDB->id_guru . "' and mapel='" . $dataDB->mapel . "' and id_tk='" . $dataDB->id_tk . "' and rpp is not null  order by id desc ")->row();
