...
class Model_pendidik extends CI_Model  {
	...
	function _get_data()
	{
		...
		$aktifasi=1;//$this->input->post("aktifasi");
		...
		$query="select * from ".$this->tbl." where id_jabatan=3 $filter ";
		...
		if(isset($_POST['order']))
		{
			if($urutkan=="1")
			{
				$query.=" order by jam_blok desc";
			}elseif($urutkan=="2")
			{
				$query.=" order by jam_valid desc";
			}else{
	...
	{				
 	...
 	public function get_nilai($id_guru){
		...
		$sms=$this->m_reff->semester();
 		...
 		$this->db->limit("1");
 		$uh = $this->db->get("data_nilai")->num_rows();
 		...
 		$this->db->where("id_semester", $sms);
 		$this->db->limit("1");
 		$uts = $this->db->get("data_nilai")->num_rows();
 		...
 		$this->db->where("id_semester", $sms);
 		$this->db->limit("1");
 		$uas = $this->db->get("data_nilai")->num_rows();
	...
	{				
	 ...
	 /*===================================*/
	function get_data_rpp()
	{
		$status=$this->input->post("status");
		$guru=$this->input->post("guru");
		
		
		$query=$this->_get_data_rpp();
		if($_POST['length'] != -1)
			if($status or $guru){
			}else{
			$query.=" limit ".$_POST['start'].",".$_POST['length'];
	...
	function _get_data_rpp()
	{
		...
		{
			$filter.="AND id_guru='".$guru."' ";
		} 
		
		 
		$query="SELECT * FROM v_mapel_ajar where id_semester='".$sms."' and id_tahun='".$tahun."'    ".$filter." GROUP BY mapel,id_tk,id_guru ";
			if($_POST['search']['value']){
			$searchkey=$_POST['search']['value'];
				$query.=" AND (
				nama LIKE '%".$searchkey."%'  or
		...
		if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls'){
         $tmp    = $_FILES[$file_form]['tmp_name']; 
				...
				if ($i > 1) {						
						...
						$nip=isset($sheet[5])?($sheet[5]):"";
						$nama=isset($sheet[1])?($sheet[1]):"";
						$nuptk=isset($sheet[6])?($sheet[6]):"";
						$alamat=isset($sheet[9])?($sheet[9]):"";
						$jk=isset($sheet[2])?($sheet[2]):"";
						$email=isset($sheet[8])?($sheet[8]):"";
						$tmt=isset($sheet[11])?($sheet[11]):"";
						$tmt=str_replace("'","",$tmt); 
						$tmt=str_replace("`","",$tmt);
						$id_pangkat=isset($sheet[4])?($sheet[4]):"";
						$id_jabatan=isset($sheet[13])?($sheet[13]):"";
						$sts_kepegawaian=isset($sheet[12])?($sheet[12]):"";
						$id_kelas=isset($sheet[10])?($sheet[10]):"";
						$id_mapel=isset($sheet[3])?($sheet[3]):"";
						 $id_ijazah=isset($sheet[14])?($sheet[14]):"";
						 $tempat_lahir=isset($sheet[15])?($sheet[15]):"";
						 $tempat_lahir=isset($sheet[15])?($sheet[15]):"";
						 $tempat_lahir=isset($sheet[15])?($sheet[15]):"";
						 
						 $tgl_lahir=isset($sheet[16])?($sheet[16]):"";
						 $tgl_lahir=str_replace("'","",$tgl_lahir); 
						 $tgl_lahir=str_replace("`","",$tgl_lahir);
						 
						 $gelar_depan=isset($sheet[17])?($sheet[17]):"";
						 $gelar_belakang=isset($sheet[18])?($sheet[18]):"";
					 
						if(!$hp){ 	 $var["hp"]=false; $var["validasi"]=false;	return $var; }
						if(!$nip){ 	 $var["nip"]=false; $var["validasi"]=false;	return $var; }
						if(!$nip){ 	 $var["nip"]=false; $var["validasi"]=false;	return $var; }
						 
						$cek_nip=$this->cek_nip($nip);
						if($cek_nip){
...
//create column
        ...
        $objPHPExcel->getActiveSheet(0)->setCellValue('D1', 'ID MAPEL YANG DIAMPU');
...
//<!-------------------------------------------------------------------------------------->	
	...
	function input_data_guru()
	{
		...
		$data=$this->security->xss_clean($input);
				...
				$this->db->set("password",md5($hp));
				$this->db->set("alias","a3".$hp."45");
