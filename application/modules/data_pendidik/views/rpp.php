 ...
 ?>
						...
						<h2>DATA RPP</h2>
                           ...
                           <div class="row clearfix">
								...
								<div class="col-md-6">
                                            ...
                                            <div class="form-line">
                                                  <select class="form-control show-tick" id="guru" data-live-search='true' onchange="reload_table()">
                                  <option value=""> === Filter Guru === </option>
                                  <?php
								  $data=$this->db->query("select * from data_pegawai where id_jabatan='3' and aktifasi='1' order by nama asc")->result();
								  foreach($data as $val)
								  {
								...
								<div class="col-md-6">
                                            ...
                                            <div class="form-line">
                                                  <select class="form-control show-tick" id="status" onchange="reload_table()">
                                  <option value=""> === Filter status  === </option>
                                  <option value="0"> Belum dicek </option>
                                  <option value="1"> RPP Valid </option>
                                  <option value="2"> RPP Tidak Valid </option>
                            ...
                            <div class="table-responsive">
                             <table id='tabel' class="tabel table-striped black table-bordered  table-hover dataTable">
								<thead  class='sadow bg-teal'  style="font-size:12px;width:100%" >			
									...
									<th class='thead' width='100px'>FILE RPP</th>
									<th class='thead' width='150px' >STATUS</th>
									
									<th class='thead'  >MAPEL</th>
									<th class='thead' >TINGKAT</th>
									<th class='thead' >GURU</th>
   ...
   var  dataTable = $('#tabel').DataTable({ 
	  ...
	  dom: 'Blfrtip',
        ...
        "ajax": {
            "url": "<?php echo site_url('data_pendidik/data_rpp');?>",
            "type": "POST",
			"data": function ( data ) {
...
function rpp(id)
{		 
			 var val = $("[name='tandai"+id+"']").val();
			 $.post("<?php echo site_url("data_pendidik/setRPP"); ?>",{id:id,val:val},function(){
			  reload_table();
		      });
}
...
{ 
  ...
  function hapus(id,judul=null){
		   ...
		   $.post("<?php echo site_url("master/hapus_pendidik"); ?>",{id:id},function(){
  ...
  function aktifasi(id,judul=null){
	  if(judul==1)
	  {
		  juduls="NON AKTIFKAN GURU INI ?";
		   ...
		   $.post("<?php echo site_url("master/aktifasi_pendidik"); ?>",{id:id,sts:judul},function(){
				notif("Data berhasil dihapus !!");			  
			  reload_table();
		      })
		   })
...
function filter()
{
	  ...
	  <!-- Modal -->
<div class="modal fade" id="mdl_formSubmit_input" tabindex="-1" role="dialog" 
...
function sudah(id)
{
		   ...
		   $.post("<?php echo base_url()?>master/buka_akun/",{id:id},function(){
			 reload_table();
		      })
		   })
 ...
 {		$(".titles").html(title);
	 	 loading();
	  $.post("<?php echo site_url("data_pendidik/cekKelas"); ?>",{id:id},function(data){
 ...
 {		$(".titles").html(title);
	 	 loading();
	  $.post("<?php echo site_url("data_pendidik/persentase"); ?>",{id:id},function(data){
			   $("#mdl_modal").modal("show");
			   $("#view").html(data);
			     $.unblockUI();
		      })
 ...
 {		$(".titles").html(title);
	 	 loading();
	  $.post("<?php echo site_url("data_pendidik/cekTidakMasuk"); ?>",{id:id},function(data){
			   $("#mdl_modal").modal("show");
			   $("#view").html(data);
			     $.unblockUI();
		      })
