 ...
 <div class="row clearfix" style="margin-top:-20px">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="area_formhonor">
		...
		$totalKBMNormal=""; $i=0; $totalKBMInvalid="";
			...
			$jml_jam_total=$jmlJamYangHadir=0;
			...
			$jamMasuk=$this->mdl->jmlJamMengajarNormal($periode,$id_guru,$tk);//jam yang masuk
				$jmlJamYangHadir=$jamMasuk+$jmlJamYangHadir;
			...
			$jml_jam_total=$jam+$jml_jam_total;
			?> Jam </td>
			<td>X&nbsp; </td>
...
$totalKBMNormal=$t+$totalKBMNormal;	///harga kbm normal
$totalKBMInvalid=$blok+$totalKBMInvalid;	///harga kbm normal
	} 
...
<?php }else{ echo "<tr><td><i>- Tidak mengajar - </i> </td></tr>"; $totalKBMNormal=0; } ?>		
			...
			<td>:</td>
			<td><?php echo number_format($pe=$this->mdl->pembinaEskul($id_guru,$periode),0,",",".");?></td>
			</tr>
			...
			<tr>
			<td>Supervisi Akademik</td>
			<td>:</td>
			<td><?php echo number_format($sa=$this->mdl->supervisi_akademik($id_guru),0,",",".");?></td>
			</tr>
			<tr>
			<td>Honor KBM Normal  </td>
			<td>:</td>
			<td><?php echo number_format($totalKBMNormal,0,",",".");?></td>
			</tr>
			<tr>
			<td>Honor KBM Inval  </td>
			<td>:</td>
			<td><?php
			$in=$this->mdl->getTotalInval($id_guru,$periode);
			//$inval=$in*$honor;
			$inval=$in;
			echo  number_format($inval,0,",",".");?>   </td>
			</tr> <tr>
			<td>Honor Non KBM </td>
			<td>:</td>
			<td><?php
			...
			<?php $totalMasuk=$honorSelainKBM+$totalKBMNormal+$kw+$pf+$pe+$piket+$wk+$tj+$gp+$sa+$inval;?>
			<tr>
			...
			$jamBlok=$jam-$jamMasuk;
			echo $blok=$this->mdl->jmlJamMengajarBlok($periode,$id_guru,$tk);
			...
			<td>BPJS</td>
			<td>:</td>
			<td style="padding:1px"><input type="text"   value="<?php echo number_format($this->mdl->getNominalBPJS($id_guru),0,",",".");?>" name="bpjs" onkeyup="hitung_pembayaran()"  onkeydown="return numbersonly(this, event);">
			 </td>
			</tr>
			<?php
			...
			<input type="hidden" name="form[inval]" value="<?php echo ($inval)?>">
			<input type="hidden" name="form[honor_mengajar]" value="<?php echo ($totalKBMNormal-$totalKBMNon)?>">
			<input type="hidden" name="form[honor_30]" value="<?php echo (($totalKBMNormal-$totalKBMNon)-($jmlJamYangHadir*$honor))?>">
			<input type="hidden" name="form[jml_jam_masuk]" value="<?php echo $jmlJamYangHadir?>">
			<input type="hidden" name="form[jml_jam_total]" value="<?php echo $jml_jam_total?>">
			<input type="hidden" name="form[honor_kbm_normal]" value="<?php echo $totalKBMNormal?>">
			...
			<input type="hidden" name="form[supervisi_akademik]" value="<?php echo $sa?>">
			...
			<input type="hidden" name="pemasukan" value="<?php echo $totalMasuk?>">
