                    ...
                    <div class="cardx">
                        <div class="header row">
						  ...
						  <div class="col-md-4  "  style="padding-bottom:10px" >
                                      <select   class="form-control show-tick id_kelas" id="id_mapel_by" name='id_mapel_by' onchange="reload_table()">
                                        <option value="">==== pilih mapel====</option>
                                         <?php
                                         	
										   //$dbkelas=$this->mdl->dataMapell();
							...
							<div class="col-md-4"   >
								...
								<div id="dataKelas">	
									<select class="form-control show-tick" id="id_kelas" data-live-search="true" onchange="reload_table()">
 ...
 $('select').selectpicker();
  ...
  <script type="text/javascript">
  	  function hapus(id,akun){
		   alertify.confirm("<center>Hapus ?</center>",function(){
		       loading();
		   $.post("<?php echo site_url("kirim_tugas/hapus"); ?>",{id:id},function(){
			   reload_table();
			    unblock();
		      })
		   })
  ...
  var  dataTable = $('#table').DataTable({ 
	...
	//		 {
        ...
        "ajax": {
            "url": "<?php echo site_url('kiriman_tugas/getCatatan');?>",
            "type": "POST",
			"data": function ( data ) {
...
function add()
{
			$.post("<?php echo site_url("kirim_tugas/viewAdd"); ?>",{},function(data){
 ...
 <div class="modal fade" id="mdl_modal_artikel" tabindex="-1" role="dialog">
	...
	<form  action="javascript:submitForm('modal_artikel')" id="modal_artikel" url="<?php echo base_url()?>kirim_tugas/insert"   method="post" enctype="multipart/form-data">
...
<script>
	 ...
	 {	 
		 		  loading();
			 $.post("<?php echo site_url("kirim_tugas/viewEdit"); ?>",{id:id},function(data){
		 	   $("#editan").html(data);
		 	     $(".tit").html(nama);
		 	       unblock(); 
 ...
 <div class="modal fade" id="mdl_modal_edit" tabindex="-1" role="dialog">
	...
	<form  action="javascript:submitForm('modal_edit')" id="modal_edit" url="<?php echo base_url()?>kirim_tugas/update"  method="post" enctype="multipart/form-data">
                    <div class="modal-content"> <span  title="tutup"  data-dismiss="modal" class="    pull-right waves-effect"><i class="material-icons">cancel</i> </span>
                        <div class="modal-header">
...
<script>
 ...
 function pilihKelasby()
 {
              var idm=$("[name='id_mapel_by']").val();
			  $.post("<?php echo site_url("kirim_tugas/getKelasByMapelnoMulty"); ?>",{idm:idm},function(data){
			  $("#dataKelas").html(data);
			  reload_table();
		      }); 
 }
function periksa(id,kelas)
{     $("#periksa").html("Loading...");
loading();
    $("#tititel").html("PERIKSA TUGAS");
    	 $.post("<?php echo site_url("kirim_tugas/periksa"); ?>",{id:id,kelas:kelas},function(data){
		 	   $("#periksa").html(data);
			    $("#mdl_periksa").modal( );
			  unblock(); 
			}); 
}
function periksa_perkelas(id,kelas,idtugas)
{ $("#tititel").html(kelas);    $("#periksa").html("Loading...");
   $.post("<?php echo site_url("kirim_tugas/periksa_perkelas"); ?>",{idkelas:id,kelas:kelas,idtugas:idtugas},function(data){
		 	   $("#periksa").html(data);
			    $("#mdl_periksa").modal( );
				 ...
				 <div class="modal fade" id="mdl_periksa" tabindex="-1" role="dialog">
                <div class="modal-dialog" id="area_modal_edit" role="document">
				
	<form  action="javascript:submitForm('modal_edit')" id="modal_edit" url="<?php echo base_url()?>kirim_tugas/periksa"  method="post" enctype="multipart/form-data">
                    <div class="modal-content"> <span  title="tutup"  data-dismiss="modal" class="    pull-right waves-effect"><i class="material-icons">cancel</i> </span>
                        <div class="modal-header">
                            <h4 class="modal-title col-teal" id="tititel">PERIKSA TUGAS</h4>
