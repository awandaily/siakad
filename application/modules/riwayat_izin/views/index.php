...
<script>
	...
	jadwalPiket();
	 function jadwalPiket() {
            $.ajax({
                url: '<?php echo base_url(); ?>riwayat_izin/process',
                type: 'POST', // Send post data
                data: 'type=fetch',
                async: false,
                success: function (s) {
...
<script> 
 function detailModal(id,title)
    {	 
       ...
       $.post("<?php echo site_url("riwayat_izin/info"); ?>",{id:id,title:title},function(data){ 
                alertify.alert(guru,data, function(){  }).set('label', 'Tutup');
            });
