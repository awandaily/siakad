...
//		return $this->m_reff->goField("tr_ektrakurikuler","id","where kode='".$dp->nip."'");
	...
	function inputNilaiG(){
		$tahun=$this->m_reff->tahun();
		 ...
		 $nilai=$this->input->post("gnilai");
		 $var=$this->input->post("f");

		 $q = $this->m_reff->goField("eskul_anggota", "j_siswa", " WHERE id_group='".$id_group."' AND id_tahun='".$tahun."' AND id_semester='".$sms."' ");

		 $anggota = json_decode($q, true);
		 //print_r($anggota);

		 foreach($anggota as $v => $val){
		 	$id_siswa = $v;
			 ...
			 if($cek){
					 ...
					 $this->db->set($var);
				 $this->db->update("tm_ekstrakurikuler");
			 }else{
					 $this->db->set("id_tahun",$tahun);
				  	 $this->db->set("id_semester",$sms);
				 	 ...
				 	 $this->db->set("id_ektra",$this->ids());
					 $this->db->set($var);
				 $this->db->insert("tm_ekstrakurikuler");
			 }
