...
$db=$this->db->query("select * from keu_tm_bayar where substr(_ctime,1,10)='".$tgl."' and id_siswa='".$id_siswa."'  order by id_tagihan asc");
if(!$db->row()){ echo "Tidak ada data" ;return false; }
$uang=$this->db->query("select sum(nominal_bayar) as bayar from keu_tm_bayar where substr(_ctime,1,10)='".$tgl."' and id_siswa='".$id_siswa."' ")->row();
$uang=$uang->bayar;
