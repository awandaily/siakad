...
class Aktivitas_guru extends CI_Controller
{
	...
	function data()
	{
		...
		$jamke 			= $this->m_reff->jam_aktif();

		$list = $this->mdl->get_open($tbl, $id_guru, $id_sts, $id_jabatan);
		$data = array();
		$no = $_POST['start'];
		$no = $no + 1;
		foreach ($list as $dataDB) {
			...
			$poto = isset($dataDB->poto) ? ($dataDB->poto) : "";
			if ($poto) {
				if (file_exists("file_upload/dp/" . $poto)) {
					$img = "<img src='" . base_url() . "file_upload/dp/" . $dataDB->poto . "' class='img-responsive thumbnail' width='100' height='100' style='margin-bottom:0' />";
				} else {
					$img = "<img src='" . base_url() . "plug/img/sample.png' class='img-responsive thumbnail' width='100' height='100' style='margin-bottom:0' />";
				}
			} else {
				$img = "<img src='" . base_url() . "plug/img/sample.png' class='img-responsive thumbnail' width='100' height='100' style='margin-bottom:0' />";
			}
			...
			if ($this->m_reff->jam_aktif() != "") {
				$this->db->like("jam", $this->m_reff->jam_aktif());
				$this->db->where("id_guru", $dataDB->id);
				$qjam_pel 	= $this->db->get("v_jadwal");

				$njam_pel	= $qjam_pel->num_rows();
				$jam_pel 	= $qjam_pel->row_array();

				if ($njam_pel != 0) {
					$mapel = $jam_pel["mapel"] . " di";
					$kelas = $jam_pel["nama_kelas"];
				} else {
			...
			if ($dataDB->sts_pegawai != "") {
				$sts = "(" . $dataDB->sts_pegawai . ")";
			} else {
				$sts = "";
			}

			$row[] = "
				<div class='row' style='min-height:150px;padding:0;margin-right:0;margin-left:0;'>
					<div class='col-md-2 col-sm-12 col-xs-12' style='height:100%;margin-bottom:0;text-align:center'>
						<span class='size linehover' style='border-radius:100%'>
							<center>
								" . $img . "
							</center>
						</span>
						<span>
						<b>	" . $dataDB->nama_lengkap . "</b><br> 
							<i>
								<!--" . $dataDB->jabatan . " <br>--->
								" . $sts . "
							</i> 
					...
					<div class='col-md-10 col-sm-12 col-xs-12' style='height:100%;margin-bottom:0;padding-left:0;padding-right:0;'>
						...
						<b>	Jam Masuk</b><br>
							" . $this->m_reff->absenDatang(date('Y-m-d'), $dataDB->nip) . "
						</div>
						<div class='col-lg-6 col-md-6 col-sm-6 col-xs-6'>
						<b>	Jam Pulang</b><br>
							" . $this->m_reff->absenPulang(date('Y-m-d'), $dataDB->nip, $this->m_reff->absenDatang(date('Y-m-d'), $dataDB->nip)) . "
		...
		$output = array(
			"draw" 				=> $_POST['draw'],
			"recordsTotal" 		=> $this->mdl->count_file($tbl, $id_guru, $id_sts, $id_jabatan),
			"recordsFiltered" 	=> $this->mdl->count_file($tbl, $id_guru, $id_sts, $id_jabatan),
			"data" 				=> $data,
		);
