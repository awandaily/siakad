 ...
 <div class="breadcrumb-header justify-content-between">
 	<div>
 		<h4 class="content-title mb-2">Hi, Ini Kirim Pengumuman</h4>
 		<nav aria-label="breadcrumb">
 			<ol class="breadcrumb">
 				<li class="breadcrumb-item"><a href="#">Aksi</a></li>
 				<li class="breadcrumb-item active" aria-current="page"> Kirim Pengumuman</li>
 			</ol>
 		</nav>
 	</div>
 				...
 				<div id="dataKelas">
 					<?php
						...
						if ($level == "ADMIN") {
							...
							echo "
												<select name='fkelas' class='form-control show-tick' onchange='reload_table()'>
 ...
 <script type="text/javascript">
 	function hapus(id, akun) {
 		alertify.confirm("<center>Hapus ?</center>", function() {
 			loading();
 			$.post("<?php echo site_url("kirim_tugas/hapus_pengumuman"); ?>", {
 				id: id
 			}, function() {
 				reload_table();
 				unblock();
 	...
 	var dataTable = $('#table').DataTable({
 		...
 		"ajax": {
 			"url": "<?php echo site_url('kirim_tugas/getPengumuman'); ?>",
 			"type": "POST",
 			"data": function(data) {
 ...
 <script>
 	function add() {
 		$.post("<?php echo site_url("kirim_tugas/viewAddPengumuman"); ?>", {}, function(data) {
 ...
 <div class="modal fade" id="mdl_modal_artikel" tabindex="-1" role="dialog">
 	<div class="modal-dialog" id="area_modal_artikel" role="document">
 		...
 		<form action="javascript:submitForm('modal_artikel')" id="modal_artikel" url="<?php echo base_url() ?>kirim_tugas/insert_pengumuman" method="post" enctype="multipart/form-data">
 ...
 <script>
 	function edit(id, nama) {
 		loading();
 		$.post("<?php echo site_url("kirim_tugas/viewEditPengumuman"); ?>", {
 			id: id
 		}, function(data) {
 			$("#editan").html(data);
 			$(".tit").html(nama);
 ...
 <div class="modal fade" id="mdl_modal_edit" tabindex="-1" role="dialog">
 	<div class="modal-dialog" id="area_modal_edit" role="document">
 		...
 		<form action="javascript:submitForm('modal_edit')" id="modal_edit" url="<?php echo base_url() ?>kirim_tugas/update_pengumuman" method="post" enctype="multipart/form-data">
 			<div class="modal-content"> <span title="tutup" data-dismiss="modal" class="    pull-right waves-effect"><i class="material-icons">cancel</i> </span>
 				<div class="modal-header">
