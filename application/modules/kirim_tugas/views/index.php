 ...
 <div class="breadcrumb-header justify-content-between">
 	<div>
 		<h4 class="content-title mb-2">Hi, Ini Kirim Tugas</h4>
 		<nav aria-label="breadcrumb">
 			<ol class="breadcrumb">
 				<li class="breadcrumb-item"><a href="#">Aksi</a></li>
 				<li class="breadcrumb-item active" aria-current="page"> Kirim Tugas</li>
 			</ol>
 		</nav>
 	</div>
 	...
 	<div class="card">
 		<div class="header row">
 			...
 			<div class="col-md-4  " style="padding-bottom:10px">
 				<select class="form-control show-tick id_kelas" id="id_mapel_by" name='id_mapel_by' onchange="pilihKelasby()">
 			...
 			<div class="col-md-4">
 				...
 				<div id="dataKelas">
 					<?php
						...
						$ray[''] = "  === pilih kelas ===  ";
						$dataray = $ray;
						echo form_dropdown("fkelas", $dataray, "", "class='form-control show-tick  '   "); ?>
 				</div>
 			</div>
 ...
 <script type="text/javascript">
 	function hapus(id, akun) {
 		alertify.confirm("<center>Hapus ?</center>", function() {
 			loading();
 			$.post("<?php echo site_url("kirim_tugas/hapus"); ?>", {
 				id: id
 			}, function() {
 				reload_table();
 				unblock();
 	...
 	var dataTable = $('#table').DataTable({
 		...
 		"ajax": {
 			"url": "<?php echo site_url('kirim_tugas/getCatatan'); ?>",
 			"type": "POST",
 			"data": function(data) {
 ...
 <script>
 	function edit(id, nama) {
 		loading();
 		$.post("<?php echo site_url("kirim_tugas/viewEdit"); ?>", {
 			id: id
 		}, function(data) {
 			$("#editan").html(data);
 			$(".tit").html(nama);
 ...
 <script>
 	function pilihKelasby() {
 		...
 		$.post("<?php echo site_url("kirim_tugas/getKelasByMapelnoMulty"); ?>", {
 			idm: idm
 		}, function(data) {
 			$("#dataKelas").html(data);
 			reload_table();
 	...
 	function periksa(id, kelas) {
 		...
 		$.post("<?php echo site_url("kirim_tugas/periksa"); ?>", {
 			id: id,
 			kelas: kelas
 		}, function(data) {
 			$("#periksa").html(data);
 	...
 	function periksa_perkelas(id, kelas, idtugas) {
 		$("#tititel").html(kelas);
 		$("#periksa").html("Loading...");
 		$.post("<?php echo site_url("kirim_tugas/periksa_perkelas"); ?>", {
 			idkelas: id,
 			kelas: kelas,
 			idtugas: idtugas
 		}, function(data) {
