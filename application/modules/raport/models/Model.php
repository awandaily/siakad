...
class Model extends CI_Model
{
	...
	function nama_wali_kelas($id_siswa, $id_tk)
	{
		...
		if ($tahun_real == $tahun_kini) {
			...
			return $this->m_reff->nama_guru($id_wali);
		} else {
	...
	function nip_wali_kelas($id_siswa, $id_tk)
	{
		$semester = $this->m_reff->semester();
		$tahun_real = $this->m_reff->tahun_asli();
		$tahun_kini = $this->m_reff->tahun();
		if ($tahun_real == $tahun_kini) {
			...
			$id_kelas = $this->m_reff->goField("data_siswa", "id_kelas", "where id='" . $id_siswa . "' ");
			$id_wali = $this->m_reff->goField("tm_kelas", "id_wali", "where id='" . $id_kelas . "' ");
			return $this->m_reff->goField("data_pegawai", "nip_asli", "where id='" . $id_wali . "' ");
		} else {
	...
	function generate_kehadiran()
	{
		...
		foreach ($d as $v) {
			...
			$sakit = $this->m_reff->jmlOffDay($v->id, 2);
			$izin  = $this->m_reff->jmlOffDay($v->id, 3);
			$alfa  = ($this->m_reff->jmlOffDay($v->id, 4) + $this->m_reff->jmlOffDay($v->id, 6));
			...
			if ($cek == 0) {
				...
				$this->db->where("id_siswa", $v->id);
				$this->db->update("tm_kh", $dt);
			}
		}
	...
	function getNilaiRataSikap($idsiswa, $mapel, $sms, $idguru) ///
	{
		...
		$return = $this->db->get("tm_sikap")->row();
		return $return = isset($return->nilai) ? ($return->nilai) : 0;
	...
	function update_dpk()
	{
		...
		$cek = $this->m_reff->goField("tm_desc_karakter", "id_siswa", "where id_semester='" . $sms . "' and id_tahun='" . $tahun . "' and id_siswa='" . $id_siswa . "' ");
		if ($cek) {
			$db = "update";
			$this->db->set($faktor, $val);
			$this->db->where("id_siswa", $id_siswa);
			$this->db->where("id_semester", $sms);
			$this->db->where("id_tahun", $tahun);
			$this->db->set("_uid", $this->idu());
		...
		return $this->db->$db("tm_desc_karakter");
	}
	function update_pkl()
	{
		$sms = $this->m_reff->semester();
	...
	function update_pkl_veto()
	{
		...
		$cek = $this->m_reff->goField("tm_pkl_veto", "id_siswa", "where id_semester='" . $sms . "' and id_tahun='" . $tahun . "' and id_siswa='" . $id_siswa . "' ");
		if ($cek) {
			$db = "update";
			$this->db->set("mitra", $mitra);
			$this->db->set("lama", $lama);
			$this->db->set("ket", $ket);
	...
	function update_catatan_k()
	{
		...
		$id_siswa = $this->input->post("id_siswa");
		$jml = $this->input->post("jml");
		$cek = $this->db->query("select * from tm_catatan_karakter where id_semester='" . $sms . "' and id_tahun='" . $tahun . "' and id_siswa='" . $id_siswa . "' ")->num_rows();
		if ($cek) {
			$db = "update";
			$this->db->set("ket", $ket);
			$this->db->where("id_siswa", $id_siswa);
		...
		return $this->db->$db("tm_catatan_karakter");
	}
	...
	{
		$nilai = $this->mdl->getNilaiAkhirRanking($idsiswa, $sms, $tk);
		if ($tk == "1") {
	...
	function setRankingAkhir($idkelas, $tk)
	{
		...
		$dbsiswa = $this->db->query("select * from data_siswa where id_kelas_$tk='" . $idkelas . "' and id_tahun_$tk=$tahun order by " . $order . "  ")->result();
		$no = 1;
		foreach ($dbsiswa as $val) {
	...
	function _getDataNilai()
	{
		...
		if ($this->m_reff->tahun_sts() == "true") {
			...
			$query = "select * from data_siswa where id_kelas='" . $this->input->post("id_kelas") . "'  and aktifasi=1 and id_tahun_keluar is null and id_sts_data in(1,4)";
		} else {
			$tahun = $this->m_reff->tahun();
	...
	function nip_kepsek($tahun)
	{
		return $this->m_reff->goField("tr_tahun_ajaran", "nip_kepsek", "where id='" . $tahun . "'");
	}
	function titiMangsaRapot($sms)
	{
		...
		//return $t;
		//return	$t=$this->m_reff->goField("tr_tahun_ajaran",$sms,"where id='".$tahun."'");
	...
	function getNsNonMuslim($idsiswa)
	{
		...
		$this->db->select("s");
		$get = $this->db->get("data_nilai_nonmuslim")->row();
		return $return = isset($get->s) ? ($get->s) : 0;
	}
