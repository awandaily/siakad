...
class Model_absen extends CI_Model  {
	...
	{
		$this->db->where("id",$this->idu());
		$db=$this->db->get($this->data_siswa)->row();
	return isset($db->nis)?($db->nis):"";
	}
	function level()
	{
	...
	{	$libur=$this->hariLibur($tgl); $titik="";
	    ...
	    $return=$this->db->get($this->tbl_siswa)->row();
		if($tgl>$hari_ini)
		{
			return "<td></td>";
		}
	...
	function dataMapel($hari,$idkelas)
	{
		return $this->db->query("SELECT * FROM ".$this->tbl_mapel." where   id_kelas=".$idkelas." and id_hari='".$hari."' order by jam_masuk asc")->result();
	}
	...
	function stsAbsenMapel($noid,$tgl,$masuk,$keluar,$id_kelas,$id_mapel)
	{	
		$getIpMapel=$this->getIpMapel($id_mapel);
